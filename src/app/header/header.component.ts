...
export class HeaderComponent {
  ...
  // sets user id, person type and token on local storage set dashboard mode
  enterDash(data, id) {
    this.loading = false;
    this.user = {
        id: data.user.id,
    ...
    // checks user admin, user with email verified
  ...
  // reset password api server
  onResetPasswordSubmit(id: string) {
    if (this.resetPasswordForm.valid) {
      // checking passwords fields validations
  ...
  // remove cities based on states array
  removeCities(state) {
