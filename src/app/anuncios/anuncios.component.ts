...
export class AnunciosComponent implements OnInit {
  ...
  // Create object needed to Pagseguro api checkout process
  checkout() {
    const data = {
      itemId1: this.itemId,
  ...
  createProp(user) {
    // creates propaganda object depending some Component props
    let propaganda;
  ...
  lightbox(code) {
    this.loading = false;
    // Pagseguro Javascript Code to open PagSeguro lightbox modal
  ...
  onSubmit() {
    ...
    if (this.registerForm.valid &&
      ...
      if (banner.files[0].type !== 'image/jpeg') {
        ...
        this.loading = true;
        // registers user to server and calls createProp after success
  ...
  populateCities() {
    if (this.stateClickedModal === undefined) {
      ...
      if (stateCode) {
        this.loading = true;
        // get cities from api and ´populate cities and cities tags arrays
  ...
  removeCities(state) {
    ...
    if (stateCode) {
      this.loading = true;
      // get cities from api and remove the included in cities and cities tags arrays
      this.citiesService.getCities(stateCode).subscribe(
        cities => {
          cities.forEach(element => {
  ...
  removeCityModal(city) {
    // Check if city selected is already on cities array
    this.cityTags = this.cityTags.filter( val => val !== city);
  }

  removeStateModal(state) {
    // Check if state selected is already on states array
  ...
  // Set State in it's correctly form (not abreviated)
  selectState(state) {
  ...
  selectStateTag(state) {
    // Convert State to His Abbreviation form
    ...
    // Populaties Cities Array after receiving state selected
  ...
  // Set properties to be used on PagSeguro Code
  ...
  // Check if there is a state selected
  showCities() {
    if (this.stateClickedModal === undefined) {
      window.alert('Selecione um Estado');
    } else {
